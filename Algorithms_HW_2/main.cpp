...
{
	tree_node_t *tmp_node;
	tree_node_t *upper_node = nullptr;
	tree_node_t *other_node = nullptr;
...
int store_leaves(tree_node_t *tree, int depth, int *storage,int total) {
	if (tree->left == NULL){ 
		...
		store_leaves(tree->left, (depth + 1), storage,total);
		store_leaves(tree->right,( depth + 1), storage,total);
	}
...
int depth_distribution(tree_node_t *tree) {
	int storage[BLOCKSIZE] = { 0 };//Initialize all elements in the array that will hold the leaves to 0;
	int total=0, depth = 0;
	int t_leaves = store_leaves(tree, depth, storage,total);//return total number of leaves through recursive function
	printf("The tree has %d leaves\n", t_leaves);
	for (int j = 0; j <= tree->height; j++) {
		if (storage[j] > 0 && j < 100){
			printf("%d leaves at depth %d\n", storage[j], j);
		}
		else if (j > 99){
